...
parser.add_argument('--dataset', type=str, default='illegal_parking')

# may contain splits
parser.add_argument('--weights', type=str, default='checkpoint/TSM_videos_1201_RGB_resnet50_shift8_blockres_avg_segment8_e120_pr8_ext0.1/ckpt.best.pth.tar')
...
# for true test
parser.add_argument('--test_list', type=str, default='/nfs/volume-95-7/temporal-shift-module/ops/test_list.txt')
...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    ...
    # net = torch.jit.load("./tsm_with_1127.pt")
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        this_label = None
        this_path = None
        for n_seg, (_, (data, label, path)), net, modality in zip(test_segments_list, data_label_pairs, net_list, modality_list):
        ...
        for p, g, n in zip(ensembled_predict, this_label.cpu().numpy(), this_path):
            output.append([p[None, ...], g, n])
...
def bad_case(args, pred, gt, path):
    save_root = '/nfs/volume-95-7/temporal-shift-module/badcase'
    right_save_root = '/nfs/volume-95-7/temporal-shift-module/rightcase'
    src_root1 = '/nfs/volume-95-7/temporal-shift-module/dataset/wenzhou_200/videos/video_8_x264'
    src_root2 = '/nfs/volume-95-7/temporal-shift-module/dataset/wenzhou_200/videos/video_25_x264'
    save_folder = os.path.join(save_root, args.weights.split('/')[-2])
    if not os.path.exists(save_folder):
        os.mkdir(save_folder)

    right_save_folder = os.path.join(right_save_root, args.weights.split('/')[-2])
    if not os.path.exists(right_save_folder):
        os.mkdir(right_save_folder)

    right_num = 0
    total_num = len(gt)
    assert len(pred) == len(gt) == len(path)
    ...
    for i, p in enumerate(pred):
        if p == gt[i]:
            ...
            video_name = path[i]
            src = os.path.join(src_root1, video_name + '.mp4')
            if not os.path.exists(src):
                src = os.path.join(src_root2, video_name + '.mp4')

            shutil.copy(src, right_save_folder)
            # print(video_name, p)
    ...
    acc = 100. * right_num / total_num
    print('My Accuracy:%.2f%%' % acc)

# analyse bad case
# bad_case(args=args, pred=video_pred, gt=video_labels, path=video_paths)
...
def precision_recall(pred, gt):
    ...
    print("My precision:", 100. * cnt / total)
...
# precision_recall(pred=video_pred, gt=video_labels)
